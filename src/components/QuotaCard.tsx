  ...
  return (
    <div>
      <div className="mb-2 flex items-baseline justify-between gap-4">
        <span
          className={cn(
            'font-semibold text-5xl tabular-nums',
      ...
      <Progress
        ...
        className={cn(
          ...
          warning === 'amber' && '*:data-[slot=progress-indicator]:bg-warning',
        )}
      />
      <div className="mt-2 flex flex-wrap items-center justify-between gap-x-4 gap-y-1 text-muted-foreground text-sm">
        {hasNumbers && (
        ...
        {burnRate != null && burnRate > 0 && (
          <span
            className={cn('tabular-nums', willExceed && 'text-destructive')}
          >
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {limits.length === 0 && !loading && !error && (
          <p className="text-muted-foreground text-sm">No quota limit found.</p>
        )}
        {limits.map((limit) => (
